...
curl -s -c cookie.txt -b cookie.txt -X POST \
--data-urlencode "lb_login%3Af_login%3Arnd=$F_LOGIN" \
--data-urlencode "lb_login%3Af_login%3At_login=$LOGIN" \
--data-urlencode "lb_login%3Af_login%3At_pwd=$PASSWORD" \
...
https://lkkbyt.mosenergosbyt.ru/common/login.xhtml > /dev/null

RESPONSE=$(curl -s -c cookie.txt -b cookie.txt https://lkkbyt.mosenergosbyt.ru/abonent/index.xhtml)
VIEWSTATE=$(echo $RESPONSE | sed -e 's/.*javax.faces.ViewState" value="//' -e 's/".*$//')
